...
import PMProButtonToo from './components/button';
...
class BlockButton2 extends Component {
	...
	render() {
		...
		return [
			...
			isSelected && (
				<form
					key="form-link"
					className={ `blocks-button__inline-link pmp-button-${buttonAlignment}`}
...
// Register the block
registerBlockType( 'pmpro/pmp-button-too', {
	title: __( 'PMpro Button Too' ),
	...
	attributes: {
		...
		buttonSize: {
			type: 'string',
			default: 'pmp-button-size-medium'
		},
		buttonShape: {
			type: 'string',
			default: 'pmp-button-shape-rounded'
		},
		buttonTarget: {
			type: 'boolean',
			default: false
